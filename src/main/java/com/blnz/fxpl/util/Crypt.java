...
public class Crypt
{
    ...
    public static String generateMD5(String src)
    {
        ...
        return encodeBytes(md5Helper.digest(src.getBytes()));
    }
     ...
     */
    private static String encodeBytes( byte[] binaryData ) 
    {
   ...
   public static final String crypt(String salt, String original)
   {
    ...
    public static String makeProductKey(String username)
    {
        if (username.length() < 3) {
            username = username + "wdl";
        } 
        // a small bit of obfuscating
        String salt = "D" + username.substring(username.length() - 2) +
            username;
        String key = crypt(salt, salt);
        return encode(key);
    }
    ...
    public static void main(String args[])
    {
        if (args.length >= 1) {
            System.out.println ("[" + args[0] + "] -> [" +
                                makeProductKey( args[0] ) + "]");
        }
    }

    public static final char PadChar = 'A';
     ...
     */
    private static final byte[] decode(byte[] data, int start, int len)
    {
        if (data == null) {
            return  null;
        }
     ...
     */
    private static final byte[] encode(byte[] data, int start, int len)
    {
        if (data == null) {
            return null;
        }
